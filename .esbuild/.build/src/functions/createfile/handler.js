...
// node_modules/aws-lambda-multipart-parser/index.js
var require_aws_lambda_multipart_parser = __commonJS({
  "node_modules/aws-lambda-multipart-parser/index.js"(exports2, module2) {
    ...
    module2.exports.parse = (event, spotText) => {
...
var write = async (filename, bucket, parse2) => {
  const params = {
    Key: filename,
    Bucket: bucket,
    Body: Buffer.from(parse2.pic.content, "ascii")
  };
...
// src/functions/createfile/handler.ts
var multipart = __toModule(require_aws_lambda_multipart_parser());
var create = async (event) => {
  if (event.isBase64Encoded) {
    ...
    try {
      ...
      const parse2 = await multipart.parse(event);
      const filename = event.pathParameters.filename;
      const bucket = "bilals3bucket";
      const newdata = await write(filename, bucket, parse2);
      if (!newdata) {
